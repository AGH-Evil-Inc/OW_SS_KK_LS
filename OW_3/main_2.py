...
class App:
    def __init__(self, master):
        ...
        method_frame.pack(pady=5)  # zmniejszony padding
        ...
        data_frame.pack(pady=5)  # zmniejszony padding
        ...
        tables_frame.pack(pady=5)  # zmniejszony padding
        
        # Tabela alternatyw
        self.alternatives_tree = ttk.Treeview(tables_frame, show='headings')
        self.alternatives_tree.pack(side=tk.LEFT, padx=5)

        # Tabela punktów odniesienia
        self.reference_tree = ttk.Treeview(tables_frame, show='headings')
        self.reference_tree.pack(side=tk.LEFT, padx=5)

        # Tabela wyników
        self.results_tree = ttk.Treeview(tables_frame, show='headings')
        self.results_tree.pack(side=tk.LEFT, padx=5)

        # Ramka na przyciski operacji
        action_frame = tk.Frame(master)
        action_frame.pack(pady=5)  # zmniejszony padding
        tk.Button(action_frame, text="Uruchom metodę", command=self.run_method).pack(side=tk.LEFT, padx=5)
        
        # Ramka na wykres
        self.plot_frame = tk.Frame(master)
        self.plot_frame.pack(pady=5, fill=tk.BOTH, expand=True)  # nie zmieniamy fill i expand, ale zmniejszamy pady
    ...
    def load_reference(self):
        ...
        if file_path:
            ...
            self.show_data_in_tree(self.reference_tree, self.reference_df)
    ...
    def run_method(self):
        ...
        # metoda wywołuje odpowiednie metody optymalizacji i wyświetla wyniki
        ...
        directions = np.ones(num_criteria)
